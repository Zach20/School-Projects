    ...
    $.ajax({
        ...
        success: function (html) {
           ...
           // likesCount.innerHTML = parseInt(likesCount.innerHTML) + 1;
            var count = parseInt(likesCount.innerHTML) + 1;
            likesCount.innerHTML = count
            var listIDs = "#" + ID + "-likes-count";
            // $(listIDs).html("").show();
            $(listIDs).html(count + ' Likes').show();
...
function DeletePost(ID, element) {
    ...
    $.ajax({
        ...
        success: function (html) {
            $(listID).html("").remove();
            $(listID).html.remove();
            element.setAttribute("onclick", "RemovePost('" + ID + "', this)");
            element.setAttribute("class", "remove-post");
        }
    });
}
//Testing...
//POST: LikeImage function
...
//POST: FlagNotification function
